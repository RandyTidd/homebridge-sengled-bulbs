...
class ElementHomeClient {
	...
	throwIfSuccessFalse(response) {
		if (response.status != 200 || !response.data.success) {
			throwData(response);
		}
	}

	throwIfRetNonZero(response) {
		if (response.status != 200 || response.data.ret != 0) {
			this.throwData(response);
		}
	}
	...
	login(username, password) {
		...
		return new Promise((fulfill, reject) => {
			...
			if (this.jsessionid != null) {
				...
				return fulfill(this.loginResponse);
			}

			if (me.debug || me.info) me.log("login via api. useAlternateLoginApi: %s", this.useAlternateLoginApi);
			...
			if (!this.useAlternateLoginApi) {
				...
				this.client.post('https://us-elements.cloud.sengled.com/zigbee/customer/login.json',
				{
					...
					this.throwIfRetNonZero(response);
	...
	getDevices() {
		...
		return new Promise((fulfill, reject) => {
			this.client.post('https://us-elements.cloud.sengled.com/zigbee/device/getDeviceDetails.json', {})
				.then((response) => {
					this.throwIfSuccessFalse(response);
